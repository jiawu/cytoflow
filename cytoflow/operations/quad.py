...
class QuadSelection(cytoflow.views.ScatterplotView):
    ...
    def plot(self, experiment, gate_name, bsave=None, **kwargs):
        ...
        super(QuadSelection, self).plot(experiment, save = None, **kwargs)
        self._ax = plt.gca()
        self._draw_lines()
        percent_dict = {}
        gate_col = experiment.data[gate_name].value_counts()/len(experiment[gate_name])
        gate_dict = dict(zip(gate_col.index,gate_col.values))

        # fill in 0 if no events in a quadrant
        for x in range(1,5):
            key = "{0}_{1!s}".format(gate_name, x)
            if key not in gate_dict:
                gate_dict[key] = 0.0

        # define percentages for each quadrant
        percent_dict['tl'] = "{0:.2g}".format(gate_dict["{0}_1".format(gate_name)])
        percent_dict['bl'] = "{0:.2g}".format(gate_dict["{0}_3".format(gate_name)])
        percent_dict['tr'] = "{0:.2g}".format(gate_dict["{0}_2".format(gate_name)])
        percent_dict['br'] = "{0:.2g}".format(gate_dict["{0}_4".format(gate_name)])



        #top left
        plt.text(0.05, 0.95,r"\textbf{{{0}}}".format(percent_dict['tl']), ha='left', va='center', transform=self._ax.transAxes, fontsize = 10)
        #top right
        plt.text(0.95, 0.95,r"\textbf{{{0}}}".format(percent_dict['tr']), ha='right', va='center', transform=self._ax.transAxes, fontsize = 10)
        #bottom left
        plt.text(0.05, 0.05,r"\textbf{{{0}}}".format(percent_dict['bl']), ha='left', va='center', transform=self._ax.transAxes, fontsize = 10)
        #bottom right
        plt.text(0.95, 0.05,r"\textbf{{{0}}}".format(percent_dict['br']), ha='right', va='center', transform=self._ax.transAxes, fontsize = 10)
        self._interactive()
        fig = plt.gcf()
        if bsave:
            if 'png' or 'svg' in bsave:
                fig.savefig(bsave, transparent=True)
                plt.close()
            else:
                with PdfPages(bsave) as pdf:
                    pdf.savefig(fig)
                    plt.close()
