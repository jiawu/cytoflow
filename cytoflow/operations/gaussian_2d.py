...
import warnings, random, string, pdb
        ...
        for group, data_subset in groupby:
            ...
            if self.sigma > 0.0:
                ...
                for c in range(0, self.num_components):
                    ...
                    qval = 2 * norm.cdf(self.sigma) - 1
                    r2 = chi2.ppf(qval, 2)

                    xl,yl = 2*np.sqrt(v[:,None]*r2)

                    # xl is the length along the x axis
                    # yl is the length along the y axis
                    #xl = 2*np.sqrt(v[0]) * self.sigma  # @UnusedVariable
                    #yl = 2*np.sqrt(v[1]) * self.sigma  # @UnusedVariable
...
class GaussianMixture2DView(HasStrictTraits):
    ...
    def plot(self, experiment, plot_name = None, get_coords = 1, get_stats = False, **kwargs):
        """
        Plot the plots.
        """
        ...
        for i, (mean, covar) in enumerate(zip(gmm.means_, gmm.covariances_)):
            ...
            r2 = chi2.ppf(2 * norm.cdf(self.op.sigma) - 1, 2)
            xl,yl = 2*np.sqrt(v[:,None]*r2)

            verts = []
            for x in range(1,9):
                el = self._plot_ellipse(mean,
                       np.sqrt(v[0] * chi2.ppf(2 * norm.cdf(x) - 1, 2)),
                       np.sqrt(v[1] * chi2.ppf(2 * norm.cdf(x) - 1, 2)),
                       180+t,
                       color = color,
                       fill = False,
                       linewidth = 1,
            ...
            vert_list.append(tuple(verts))
            mean_list.append((i,mean))
            cvar_list.append((i,covar))

        if type(get_coords) == str:
            # string could be top-left, top-right, bottom-left, bottom-right
            # get all the means

            # if top, get the highest

            # corners are from top left, clockwise
            xsc = (self.op._xscale(plt.gca().get_xlim()[0]),self.op._xscale(plt.gca().get_xlim()[1]))
            ysc = (self.op._yscale(plt.gca().get_ylim()[0]),self.op._yscale(plt.gca().get_ylim()[1]))

            corners = { 'tl': (xsc[0], ysc[1]),
                        'tr': (xsc[1], ysc[1]),
                        'bl': (xsc[0], ysc[0]),
                        'br': (xsc[1], ysc[0]) }

            target_corner = corners[get_coords]
            mean_list.sort(key = lambda p: math.sqrt((p[1][0] - target_corner[0])**2 + (p[1][1] - target_corner[1])**2))
            coords = vert_list[mean_list[0][0]][int(self.op.sigma-1)]
        elif type(get_coords) == int:
            coords = vert_list[int(get_coords-1)][int(self.op.sigma-1)]
        #self.f_axes = plt.gca()
        if get_stats:
            converted_mean = self.op._yscale.inverse(mean_list[0][1])
            return(coords, converted_mean, cvar_list[mean_list[0][0]][1], xl, yl)
        else:
            return(coords)
